...
WorkflowRefreshHandler = function(htmlElementId) {
...
// Specifies that the custom Ajax handler uses the prototype provided by EBX5
WorkflowRefreshHandler.prototype = new EBX_AJAXResponseHandler();
...
function ebx_wfw_callAjaxComponent(urlToAjaxComponent, htmlElementId, requestParameters) {
	...
	var myRefreshHandler = new WorkflowRefreshHandler(htmlElementId);
...
function ebx_wfw_changeTaskTerminationCriteria(selectedValue, divId) {
	...
	if (EBXUtils.arrayContains(ebx_wfw_tabTerminationCriteria_ignoreReject, selectedValue.key)) {
