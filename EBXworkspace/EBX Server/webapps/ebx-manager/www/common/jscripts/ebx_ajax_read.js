...
var onAjaxMsgDivId = 'onAjaxMsgDiv';

function onAjaxHideMessageDiv() {
	var msgDiv = document.getElementById(onAjaxMsgDivId);
	msgDiv.style.display = 'none';
	document.body.removeChild(msgDiv);
	document.body.removeChild(document.getElementById(onAjaxMsgDivId + "_bg"));
}

// EBX5 Ajax function (limited to read only and display html)
function EBX_Ajax() {
	...
	this.setReadyStateChangeObject = function(anObject) {
		if (!anObject instanceof EBX_AbstractAjaxResponseManager) {
			EBXLogger.log("EBX_Ajax.setReadyStateChangeObject: Given object must implement Javascript prototype [EBX_AbstractAjaxResponseManager]",
					EBXLogger.error);
			return;
		}
	...
	this.execute = function(anUrl) {
		...
		ebx_LoadingSupervisor_AJAXRequestSent();
		...
		EBXLogger.log("EBX_Ajax.execute: Request send at url [" + anUrl + "]", EBXLogger.info);
		*/
		...
		EBXLogger.log("EBX_Ajax.execute: Request send at url [" + url + "] with params [" + params + "]", EBXLogger.info);
	};
...
//All Ajax functionnalities in EBX5 must implement this prototype !!
...
function EBX_AbstractAjaxResponseManager() {
	...
	this.onExecuteIfOK = function(responseXML, root) {
		EBXLogger.log("EBX_AbstractAjaxResponseManager.onExecuteIfOK: Ajax object must implement method [ onExecuteIfOK(responseXML, root) ]",
				EBXLogger.error);
		return false;
	};
	...
	this.onExecuteIfKO = function(responseXML) {
		EBXLogger.log("EBX_AbstractAjaxResponseManager.onExecuteIfKO: Ajax object must implement method [ onExecuteIfKO(responseXML, root) ]",
				EBXLogger.error);
	};
	...
	this.onCheckExpectedResponseCode = function(aResponseCode) {
		switch (aResponseCode) {
			...
			default:
				EBXLogger.log("EBX_AbstractAjaxResponseManager.onCheckExpectedResponseCode: Unknown response code [" + aResponseCode
						+ "]. Response text: [ " + this.getResponseText() + " ]", EBXLogger.error);
				return false;
		}
	};
	...
	this.onDisplayMessage = function(message) {
		...
		if (isHidden) {
			...
			/*
			 ...
			 EBXLogger.log(message.firstChild.data, EBXLogger.info);
			 if (EBX_UserMessageManager.WARNING === severity)
			 EBXLogger.log(message.firstChild.data, EBXLogger.warn);
			 if (EBX_UserMessageManager.ERROR === severity)
			 EBXLogger.log(message.firstChild.data, EBXLogger.error);
			 return;
			 */
		}
	...
	//Manage an HTTP error response (not an EBX Ajax error response document, see "onExecuteIfKO(...)" for that)
	this.onManageErrorResponse = function() {};

	this.onManageTimeoutResponse = function() {
		EBXLogger.log("EBX_AbstractAjaxResponseManager.onManageTimeoutResponse: Time out", EBXLogger.warn);
