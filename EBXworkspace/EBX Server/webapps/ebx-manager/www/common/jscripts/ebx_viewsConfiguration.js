function onSelectLevel(newLevel, submitButtonId) {
...
function onDblClick(aSelect) {
	...
	var selectedOption = aSelect.options[aSelect.selectedIndex];
	if (selectedOption.disabled) {
		return;
	}
	currentInputField.value = currentInputField.value + "${." + selectedOption.value + "}";
	onHideShowHelper(null, null);
}

var ebx_helperPanel;
function onHideShowHelper(helperDivId, inputFieldId) {
	var mainHelperDiv = document.getElementById('onLevelLabelHelper');
	if (ebx_helperPanel != null && ebx_helperPanel.cfg.getProperty('visible') == true) {
		currentInputField = null;
		ebx_helperPanel.hide();
	} else {
		var aSelectDiv = document.getElementById(helperDivId);
		var theSelect = aSelectDiv.getElementsByTagName('SELECT')[0];
		var theHelperSelect = mainHelperDiv.getElementsByTagName('SELECT')[0];
		var matchingInputField = document.getElementById(inputFieldId);

		currentInputField = matchingInputField;
		onCopyOptions(theSelect, theHelperSelect);

		var bodyEl = document.body;
		if (ebx_helperPanel == null) {
			ebx_helperPanel = new YAHOO.widget.Panel('onLevelLabelHelper_panel', {
				visible: false,
				close: true,
				draggable: false
			});
			ebx_helperPanel.setBody(mainHelperDiv.innerHTML);
			ebx_helperPanel.render(bodyEl);
		}

		ebx_helperPanel.cfg.setProperty('context', [ matchingInputField, 'tl', 'bl' ]);
		ebx_helperPanel.show();
	}
}

var ebx_helperTreePanel;
var ebx_helperTreePanelId = "ebx_helperTreePanel";

function onSelectPathTreeHelper(value, concat) {
	if (currentInputField == null) {
		return;
	}

	if (concat) {
		EBX_Form.insertTextAtCursorPosition(currentInputField, value);
	} else {
...
function onHideShowTreeHelper(helperDivId, inputFieldId, buttonId, pos1, pos2) {
	...
	if (ebx_helperTreePanel == null) {
		...
		ebx_helperTreePanel = new YAHOO.widget.Panel(ebx_helperTreePanelId, {
			visible: false,
			close: false,
			draggable: false,
			height: "150px",
	...
	ebx_helperTreePanel.cfg.setProperty('context', [ inputFieldId, pos1, pos2 ]);
	ebx_helperTreePanel.show();
}

function onCopyOptions(from, to) {
	clearSelect(to);
	to.newOption = function(text, value, disabled) {
		var newOpt = this.appendChild(document.createElement("OPTION"));
		newOpt.text = text;
		newOpt.value = value;
		newOpt.disabled = disabled;
		if (disabled) {
			newOpt.style.fontStyle = 'italic';
			newOpt.style.color = '#555555';
		}
	};
	for ( var i = 0; i < from.length; i++) {
		var anOption = from.options[i];
		to.newOption(anOption.text, anOption.value, anOption.disabled);
	}
}

function clearSelect(aSelect) {
	for ( var i = aSelect.length - 1; i >= 0; i--) {
		var achild = aSelect.options[i];
		aSelect.removeChild(achild);
	}
}

function onWBP_ajaxSelectRecordObject(aCheckBox, aRowId, isHierarchyApplied) {
	...
	this.updateLabel = function() {
		var selectReportId = EBX_Table.selectReportRegister[ebx_workspaceTableName];

		if (selectReportId === undefined) {
			EBXLogger.log("onWBP_ajaxSelectRecordObject.updateLabel: select report not found.", EBXLogger.error);
			return;
		}

		var selectReportNumber = document.getElementById(selectReportId + EBX_Table.constants.suffixSelectionNumber);
		var selectReportLabelOne = document.getElementById(selectReportId + EBX_Table.constants.suffixSelectionLabelOne);
		var selectReportLabelMany = document.getElementById(selectReportId + EBX_Table.constants.suffixSelectionLabelMany);
...
onWBP_ajaxSelectRecordObject.prototype = new EBX_AbstractAjaxResponseManager();
...
EBX_ViewsManagement.dualListClick = function(event, listEl) {
	...
	var tableEl = EBXUtils.getFirstParentMatchingTagName(targetEl, "table");
	if (tableEl == null) {
		return;
	}

	// Focusing TR element
	var trEl = EBXUtils.getFirstParentMatchingTagName(targetEl, "tr");
	if (trEl == null) {
		trEl = targetEl;
	}
...
EBX_DualList.switchOrderingMethod = function(checkboxEl) {
	var selectedColumnsEl = EBXUtils.getFirstDirectChildMatchingCSSClass(checkboxEl.parentNode.parentNode, "ebx_selectedColumnsList");
	var selectedOptionsEl = EBXUtils.getFirstDirectChildMatchingCSSClass(checkboxEl.parentNode.parentNode, "ebx_hidden_selectedColumnsList");
	...
	if (checkboxEl.checked) {
		...
		EBXUtils.addCSSClass(selectedColumnsEl, "ebx_InactiveSelectedColumnsList");
	} else {
		EBX_DualList.isUsingDataModelOrdering = false;
		EBXUtils.removeCSSClass(selectedColumnsEl, "ebx_InactiveSelectedColumnsList");
	}
...
EBX_DualList.sortElementsAccordingToDMAOrder = function(selectedColumnsEl, selectedOptionsEl) {
	var availableColumnItems = EBXUtils.getFirstDirectChildMatchingCSSClass(selectedColumnsEl.parentNode, "ebx_availableColumnsList").children;
	var availableOptionItems = EBXUtils.getFirstDirectChildMatchingCSSClass(selectedColumnsEl.parentNode, "ebx_hidden_availableColumnsList").children;
	...
	for ( var i = 0; i < available_length; i++) {
		if (EBXUtils.matchCSSClass(availableColumnItems[i], "ebx_HiddenListElement")) {
			var columnNodeToCopy = availableColumnItems[i].cloneNode(true);
			EBXUtils.removeCSSClass(columnNodeToCopy, "ebx_HiddenListElement");
...
EBX_DualList.switchMiddleCtrlButtonsState = function(dualListContainerEl) {
	var selectedColumnsCtrlsEl = EBXUtils.getFirstDirectChildMatchingCSSClass(dualListContainerEl, "ebx_selectedColumnsCtrls");
	if (selectedColumnsCtrlsEl === null) {
		return;
	}

	var moveTopButton = EBXUtils.getFirstDirectChildMatchingCSSClass(selectedColumnsCtrlsEl, "ebx_MoveTop");
	var moveUpButton = EBXUtils.getFirstDirectChildMatchingCSSClass(selectedColumnsCtrlsEl, "ebx_MoveUp");
	var moveDownButton = EBXUtils.getFirstDirectChildMatchingCSSClass(selectedColumnsCtrlsEl, "ebx_MoveDown");
	var moveBottomButton = EBXUtils.getFirstDirectChildMatchingCSSClass(selectedColumnsCtrlsEl, "ebx_MoveBottom");
...
EBX_DualList.switchFixedColumnsSelectorState = function(dualListContainerEl) {
	var fixedColumnsSelectorEl = EBXUtils.getFirstDirectChildMatchingCSSClass(dualListContainerEl, "ebx_fixColumnsCountSelector");
	...
	if (btnClassName.indexOf("ebx_MoveAllLeft") > -1) {
		if (!EBX_DualList.isUsingDataModelOrdering) {
			...
			for ( var i = 0; i < available_length; i++) {
				...
				if (!EBXUtils.matchCSSClass(currentAvailableEl, "ebx_HiddenListElement")) {
			...
			var selectedLi = EBXUtils.getFirstDirectChildMatchingCSSClass(selectedListEl, "ebx_Selected");
			...
			for ( var i = 0; i < available_length; i++) {
				...
				if (EBXUtils.matchCSSClass(currentToCopyEl, "ebx_HiddenListElement")) {
					EBXUtils.removeCSSClass(currentToCopyEl, "ebx_HiddenListElement");
				} else {
					EBXUtils.addCSSClass(availableListEl.children[i], "ebx_HiddenListElement");
				}
		...
		// workaround for firstElementChild (IE8)
		var firstLi = EBXUtils.getFirstDirectChildMatchingTagName(availableListEl, "li");
...
EBX_DualList.swapListNodes = function(btnClassName, cntnrId) {
	container = document.querySelector("#" + cntnrId);
	selectedColumnsEl = container.querySelector(".ebx_hidden_selectedColumnsList");
	optionsEls = selectedColumnsEl.options;

	selectedListEl = container.querySelector(".ebx_selectedColumnsList");
	selectedItemsEls = selectedListEl.querySelectorAll("li");
	selectedIdx = EBX_DualList.getFocusedLineIndex(selectedListEl);
...
EBX_DualList.getFocusedLineIndex = function(listEl) {
	focusedEl = listEl.querySelector(".ebx_Selected");
	if (focusedEl === null) {
		return -1;
	}
	return EBXUtils.getIndex(focusedEl);
};
...
EBX_DualList.focusListLine = function(lineEl) {
	...
	parentListEl = lineEl.parentNode;
	if (parentListEl == null) {
		return;
	}

	listLineEls = parentListEl.querySelectorAll(".ebx_Selected");
	if (listLineEls != null) {
...
EBX_DualList.reverseOrder = function(orderCtrlEl, hiddenSelectName) {
	...
	var orderLabelEl = EBXUtils.getFirstRecursiveChildMatchingCSSClass(orderCtrlEl, "ebx_SortingColumnLabel");
	var orderButtonEl = EBXUtils.getFirstRecursiveChildMatchingCSSClass(orderCtrlEl, "ebx_AscendingColumn");
	if (orderButtonEl == null) {
		isAscending = false;
		orderButtonEl = EBXUtils.getFirstRecursiveChildMatchingCSSClass(orderCtrlEl, "ebx_DescendingColumn");
	}
	...
	var list = EBXUtils.getFirstParentMatchingTagName(orderCtrlEl, "ul");
	var index = EBX_DualList.getFocusedLineIndex(list);

	var hiddenSelect = EBXUtils.getFirstRecursiveChildMatchingCSSClass(list.parentNode, "ebx_hidden_selectedColumnsList");
...
EBX_DualList.dualListClick = function(event, listEl) {
	...
	var ulEl = EBXUtils.getFirstParentMatchingTagName(targetEl, "ul");
	if (ulEl == null) {
		return;
	}
	...
	// Focusing LI element
	var liEl = EBXUtils.getFirstParentMatchingTagName(targetEl, "li");
	if (liEl == null) {
		liEl = targetEl;
	}
	...
	var sortEl = targetEl;
	var isSortingEl = EBXUtils.matchCSSClass(sortEl, "ebx_SortingColumnButtonContainer");
	if (!isSortingEl) {
		sortEl = EBXUtils.getFirstParentMatchingCSSClass(targetEl, "ebx_SortingColumnButtonContainer");
	}
	if (sortEl != null) {
